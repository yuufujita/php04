...
if ($status==false) {
  ...
  if(isset($_SESSION['kanri_flg']) && ($_SESSION['kanri_flg']===0 || $_SESSION['kanri_flg']===1)){
    ...
    $view .= '<table>';
    $view .= '<tr>';
    $view .= '<td class="left">'.'投稿者'.'</td>';
    $view .= '<td class="right">'.h($result['user_nm']).'</td>';
    $view .= '</tr>';
    $view .= '<tr>';
...
Microsoft.Maps.loadModule(
  ...
  function () {
    ...
    console.log(lat,'初期表示内容でDBの最終レコードlat、phpファイルの129行目');
    console.log(lon,'初期表示内容でDBの最終レコードlon、phpファイルの130行目');
...
function getLonLatData(e){
  ...
  const lat = e.parentNode.childNodes[9].defaultValue
  const lon = e.parentNode.childNodes[10].defaultValue
  console.log("164行目");
  console.log(lat,'165行目');
  console.log(lon,'166行目');
